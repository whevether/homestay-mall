      ...
      <nut-tab-pane :title="item" v-for="(item, index) in tabArr" :key="index">
        ...
        <scroll-view :scroll-y="true" style="height: 80vh;" @scrolltolower="onLower" class="shop-list">
          <view class="shop-item" v-if="list.length > 0" v-for="(item, key) in list" :key="key"
            @click="pageToUrl(`/pages/category/details/index?id=${item?.id}&name=${item?.name}`)">
...
export default {
  ...
  setup() {
    ...
    const onGetCategoryList = (pageNo, sort, addressCode) => {
      http.post('/api/cms/getContentDetailPageList', {
        pageNo: pageNo,
        pageSize: 10,
        sort: sort,
        extends: { categoryId: state?.routerParams?.id }
      }).then(res => {
    ...
    const onLower = () => {
      if (!state.finished) {
        onGetCategoryList(state.pageNo, state.tab, commonStore?.address?.addressCode);
      }
    };
    useDidShow(() => {
      Taro.setNavigationBarTitle({title: `${state?.routerParams?.name}列表`});
      onGetCategoryList(state.pageNo, state.tab, commonStore?.address?.addressCode);
    });
